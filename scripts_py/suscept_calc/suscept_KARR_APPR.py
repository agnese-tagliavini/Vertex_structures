...
#fRG 2loop karrasch approximation data 
...
def f_upup_fun_ph(i,j,k):
    if isInside(i,j,k):    
        ...
        return K_upup_ph(i) + K_upup_xph(PHtoXPH((i,j,k))[0]) + K_upup_pp(PHtoPP((i,j,k))[0]) 

def f_upup_fun_pp(i,j,k):
    if isInside(i,j,k):
        return f_upup_fun_pf(PPtoPF((i,j,k))[0], PPtoPF((i,j,k))[1], PPtoPF((i,j,k))[2])
    else:
        return  K_upup_pp(i) + K_upup_ph(PPtoPH((i,j,k))[0]) + K_upup_xph(PPtoXPH((i,j,k))[0])
...
def f_upup_fun_xph(i,j,k):
    if isInside(i,j,k):
        ...
        return  K_upup_xph(i) + K_upup_ph(XPHtoPH((i,j,k))[0]) + K_upup_pp(XPHtoPP((i,j,k))[0])

def f_updo_fun_ph(i,j,k):
    if isInside(i,j,k):    
        return f_updo_fun_pf(PHtoPF((i,j,k))[0], PHtoPF((i,j,k))[1], PHtoPF((i,j,k))[2])
    else:
        return - U + K_updo_ph(i) + K_updo_xph(PHtoXPH((i,j,k))[0]) + K_updo_pp(PHtoPP((i,j,k))[0]) 

def f_updo_fun_pp(i,j,k):
    if isInside(i,j,k):
        return f_updo_fun_pf(PPtoPF((i,j,k))[0], PPtoPF((i,j,k))[1], PPtoPF((i,j,k))[2])
    else:
        return - U + K_updo_pp(i) + K_updo_ph(PPtoPH((i,j,k))[0]) + K_updo_xph(PPtoXPH((i,j,k))[0])
...
def f_updo_fun_xph(i,j,k):
    if isInside(i,j,k):
        ...
        return - U + K_updo_xph(i) + K_updo_ph(XPHtoPH((i,j,k))[0]) + K_updo_pp(XPHtoPP((i,j,k))[0])
